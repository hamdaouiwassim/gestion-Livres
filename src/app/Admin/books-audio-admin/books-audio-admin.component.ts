...
@Component({
  selector: 'app-books-audio-admin',
  templateUrl: './books-audio-admin.component.html',
  styleUrls: ['./books-audio-admin.component.css']
})
export class BooksAudioAdminComponent implements OnInit {
  ...
  getAllBooks(){
   //const id = localStorage.getItem('userid')
    axios.get('http://127.0.0.1:8000/api/books/audio',{ headers: {"Authorization" : `Bearer ${this.token}`}} ).then(res=>{
      this.books = res.data;
    }).catch(err=>{
      console.log(err);
    })
  ...
  AddBook( f : NgForm){
    ...
    data.append('son',this.selectedAudio)
    console.log(data)
